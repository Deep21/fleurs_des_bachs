...
abstract class REST_Controller extends CI_Controller
{
    ...
    */
    ...
    {
        return $this->rest->db->insert(config_item('rest_logs_table'), array('uri' => $this->uri->uri_string(), 'method' => $this->request->method, 'params' => $this->_args ? (config_item('rest_logs_json_params') ? json_encode($this->_args) : serialize($this->_args)) : null, 'api_key' => isset($this->rest->key) ? $this->rest->key : '', 'ip_address' => $this->input->ip_address(), 'time' => function_exists('now') ? now() : time(), 'authorized' => $authorized));
    }
    ...
    protected function nardine()
    {
    }

    private function isCookieExist()
    {
        $this->load->helper('cookie');
        $ci = get_instance();
        $ci_session = $this->input->cookie($ci->config->item('sess_cookie_name'), TRUE);
        if ($ci_session != NULL) return true;
    }
}
